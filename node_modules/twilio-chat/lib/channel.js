...
__decorate([
    typecheck_1.asyncTypeCheck([
        'string',
        typecheck_1.literal(null),
        // Wrapping it into a custom rule is necessary because the FormData class is not available on initialization.
        typecheck_1.custom((value) => [value instanceof FormData, 'an instance of FormData']),
        // This custom type check validates a media options object.
        typecheck_1.custom((value) => {
            const baseDescription = 'valid media options';
            if (typeof value !== 'object' || value === null || Array.isArray(value)) {
                return [false, `${baseDescription} (should be a pure object)`];
            }
            const validContentType = typeof value.contentType === 'string' && value.contentType.length > 0;
            if (!validContentType) {
                return [
                    false,
                    `${baseDescription} (mediaOptions.contentType should be a non-empty string)`,
                    `malformed media options (mediaOptions.contentType is ${typecheck_1.stringifyReceivedType(value.contentType)})`
                ];
            }
            let validMedia = (typeof value.media === 'string' && value.media.length > 0)
                || value.media instanceof Uint8Array
                || value.media instanceof ArrayBuffer;
            ...
            if (!validMedia) {
                return [
                    false,
                    `${baseDescription} (mediaOptions.media should be a non-empty string, an instance of Buffer or an instance of Blob)`,
                    `malformed media options (mediaOptions.media is ${typecheck_1.stringifyReceivedType(value.media)})`
                ];
            }
            return [true];
        })
    ], ['undefined', 'string', 'number', 'boolean', 'object', typecheck_1.literal(null)]),
    __metadata("design:type", Function),
